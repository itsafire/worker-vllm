...
RUN if [[ "${WORKER_CUDA_VERSION}" == 11.8* ]]; then \
        ...
        python3.11 -m pip install -e git+https://github.com/runpod/vllm-fork-for-sls-worker.git@old-cuda11.8#egg=vllm; \
    else \
        python3.11 -m pip install -e git+https://github.com/runpod/vllm-fork-for-sls-worker.git@old-cuda12.1#egg=vllm; \
    fi && \
